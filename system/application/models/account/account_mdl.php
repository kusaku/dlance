...
class Account_mdl extends Model {
	...
	function get_payment($id) {
		...
		switch ($query['status']) {
			case 1:
				$query['status'] = 'Ожидание';
				break;
			case 2:
				$query['status'] = 'Завершён';
				break;
			case 3:
				$query['status'] = 'Возвращен';
				break;
		}
	...
	function get_payments($start_from = FALSE, $per_page, $user_id) {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			switch ($query[$i]['status']) {
				case 1:
					$query[$i]['status'] = 'Ожидание';
					break;
				case 2:
					$query[$i]['status'] = 'Завершён';
					break;
				case 3:
					$query[$i]['status'] = 'Возвращен';
					break;
			}
			...
			switch ($query[$i]['type']) {
				case 1:
					$query[$i]['type'] = 'Прямой платеж';
					break;
				case 2:
					$query[$i]['type'] = 'С протекцией сделки';
					break;
			}
		}
	...
	function cancel_applications($id, $user_id) {
		...
		$query = $this->db->get_where('balance_applications', array(
			'id '=>$id
		));
	...
	function count_services($user_id = '') {
	...
	function up_portfolio($id, $user_id) {
		...
		$query = $this->db->get_where('portfolio', array(
			'id'=>$id
		))->row_array();
		...
		if ($this->db->count_all_results('portfolio') > 0) {
			...
			$this->db->update('portfolio', array(
				...
				'position'=>$replace_position,'user_id'=>$user_id
			));
