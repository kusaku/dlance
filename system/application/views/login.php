			...
			$(document).ready(function(){
				...
				$('#login-form').submit(function(event){
					...
					if (!login || login.length == 0) {
						$('#login-block').removeBlockMessages().blockMessage('Пожалуйста, введите логин', {
							type: 'warning'
						});
					}
					else 
						if (!pass || pass.length == 0) {
							$('#login-block').removeBlockMessages().blockMessage('Пожалуйста, введите пароль', {
								type: 'warning'
							});
						}
						else {
							...
							$.ajax({
								...
								success: function(json){
									if (json.status == "OK") {
										...
										// Message
										$('#login-block').removeBlockMessages().blockMessage(data.error || 'Неверно введён логин или пароль', {
											type: 'error'
										});
							...
							// Message
							$('#login-block').removeBlockMessages().blockMessage('Пожалуйста подождите, проверка данных...', {
								type: 'loading'
							});
						}
				});
				...
				<div class="block-content">
					...
					<form class="form with-margin" name="login-form" id="login-form" method="post" action="">
						...
						<p class="inline-small-label">
							<label for="login">
								<span class="big">Логин</span>
							</label>
							<input type="text" name="login" id="login" class="full-width" value=""></p>
						<p class="inline-small-label">
							<label for="pass">
								<span class="big">Пароль</span>
							</label>
							<input type="password" name="pass" id="pass" class="full-width" value=""></p>
						...
						<p class="input-height">
							...
							<label for="keep-logged" class="inline">Запомнить</label>
						</p>
					</form>
					<form class="form" id="password-recovery" method="post" action="">
						<fieldset class="grey-bg no-margin collapse">
							...
							<p class="input-with-button">
								<label for="recovery-mail">Введите e-mail адрес</label>
