	...
	<article class="container_12">
		...
		<section class="grid_12">
			<div class="block-border"><form class="block-content form" id="table_form" method="post" action="/administrator/designs_action">
				<h1>Дизайны</h1>
...
<? foreach($data as $row): ?>
						<tr>
							<th scope="row" class="table-check-cell"><input type="checkbox" name="designs[]" value="<?=$row['id']?>" /></th>
							<td><a href="/designs/<?=$row['id']?>.html" target="_new"><?=$row['title']?></a></td>
							<td><?=$row['category']?></td>
							<td><a href="<?=$row['full_image']?>"><small><img src="/templates/admin/images/icons/fugue/image.png" width="16" height="16" class="picto"></small></a></td>
							<td><?=$row['date']?></td>
							<td><?=$row['moder']?></td>
							<td><?=$row['status']?></td>
						</tr>
