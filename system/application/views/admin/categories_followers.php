...
<div><span class="column-sort"> <a
	href="/administrator/categories_followers/?order_field=balance<? if( !empty($url) ): ?>&<?=$url?><? endif;?>"
	title="Sort up"
	class="sort-up<? if( $input['order_field'] == 'balance' and  $input['order_type'] == 'desc' ): ?> active<? endif;?>"></a>

<a
	href="/administrator/categories_followers/?order_field=balance&order_type=asc<? if( !empty($url) ): ?>&<?=$url?><? endif;?>"
	title="Sort down"
	class="sort-down<? if( $input['order_field'] == 'balance' and  $input['order_type'] == 'asc' ): ?> active<? endif;?>"></a>
</span> ������</div>
<div><span class="column-sort"> <a
	href="/administrator/categories_followers/?order_field=rating<? if( !empty($url) ): ?>&<?=$url?><? endif;?>"
	title="Sort up"
	class="sort-up<? if( $input['order_field'] == 'rating' and  $input['order_type'] == 'desc' ): ?> active<? endif;?>"></a>

<a
	href="/administrator/categories_followers/?order_field=rating&order_type=asc<? if( !empty($url) ): ?>&<?=$url?><? endif;?>"
	title="Sort down"
...
<? foreach($data as $row): ?>
	<li>
	<div class="grid-picto user"><small><?=$row['surname']?> <?=$row['name']?></small>
	<p class="grid-name"><?=$row['username']?></p>
	<p class="grid-details">������: <b><?=$row['balance']?></b><br>
	�������: <b><?=$row['rating']?></b></p>
	</div>
	<ul class="grid-actions">
		<li><a href="/administrator/users_edit/<?=$row['id']?>"
			title="�������������" class="with-tip"><img
			src="/templates/admin/images/icons/fugue/pencil.png" width="16"
			height="16"></a></li>
		<li><input type="checkbox" name="selected[]" id="grid-selected-1"
			value="1"></li>
	</ul>
	</li>
	<? endforeach; ?>
