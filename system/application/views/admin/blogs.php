	...
	<article class="container_12">
		...
		<section class="grid_12">
			<div class="block-border"><form class="block-content form" id="table_form" method="post" action="/administrator/blogs_action">
				<h1>Записи</h1>
...
<? foreach($data as $row): ?>
						<tr>
							<th scope="row" class="table-check-cell"><input type="checkbox" name="blogs[]" value="<?=$row['id']?>" /></th>
							<td><?=$row['title']?></td>
							<td><?=$row['category']?></td>
							<td><?=$row['date']?></td>
							<td><?=$row['username']?></td>
