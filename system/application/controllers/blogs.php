...
class Blogs extends Controller {
	...
	function view($id = '') {
		...
		if ($this->input->post('newcomment') and $this->users_mdl->logged_in()) {
			...
			$commentdata = array(
				'date'=>now(),'blog_id'=>$id,'user_id'=>$this->user_id,'text'=>$this->input->post('text')
			);
	...
	function add() {
		...
		$data = array(
			'date'=>now(),'title'=>$this->input->post('title'),'user_id'=>$this->user_id,
				'text'=>htmlspecialchars($this->input->post('text')),'descr'=>character_limiter($this->input->post('text'), 255),
	...
	function edit($id) {
		...
		$blogdata = array(
			'title'=>$this->input->post('title'),'text'=>htmlspecialchars($this->input->post('text')),
				'descr'=>character_limiter($this->input->post('text'), 255),'category'=>$this->input->post('category_id')
		);
	...
	function _check($id = '', $user_id = '') {
		$userdata = $this->users_mdl->get_user($user_id);
		
		//Если блог модератора
		if ($userdata['team'] == 2) {
			if (!$this->_check_action($id)) {
				return FALSE;
			}
		}
		//Блог обычного пользователя (может редактировать модератор)
		else {
